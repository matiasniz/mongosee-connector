...
module.exports = function (strapi) {
  ...
  function initialize() {
    const connectionsPromises = mongooseConnections.map(
      async (connectionName) => {
        ...
        if (pathCert && pathCert.length > 0) {
          ca = [fs.readFileSync(pathCert)];
        }
         ...
         */
          if (pathCert && pathCert.length > 0) {
            await instance.connect(
              `${connectionString}${database}?ssl=true&retryWrites=true&w=majority`,
              {
                sslValidate,
                sslCA: ca,
                useNewUrlParser: true,
